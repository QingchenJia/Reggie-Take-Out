...
public class LoginCheckInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        Long userIdEmployee = (Long) request.getSession().getAttribute("employee");
        Long userIdUser = (Long) request.getSession().getAttribute("user");

        // 如果用户ID为空，表示用户未登录
        if (userIdEmployee == null && userIdUser == null) {
            // 记录警告日志，提示用户未登录
            log.warn("用户未登录");
        ...
        if (userIdEmployee != null) BaseContext.setCurrentId(userIdEmployee);
        if (userIdUser != null) BaseContext.setCurrentId(userIdUser);
